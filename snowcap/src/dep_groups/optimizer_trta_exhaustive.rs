...
//! # Dependency-pruned exhaustive TRTA optimizer
...
/// found a valid solution. Unlike [`OptimizerTRTA`](crate::optimizers::OptimizerTRTA), this
/// variant records complete solutions and keeps exploring all dependency-pruned branches until the
/// search space is exhausted or the time budget expires.
pub struct OptimizerTRTAExhaustive<P>
where
    P: SoftPolicy + Clone,
{
    net: Network,
...
impl<P> Optimizer<P> for OptimizerTRTAExhaustive<P>
where
    P: SoftPolicy + Clone,
{
    fn new(
        ...
        loop {
            ...
            let frame = match stack.last_mut() {
                ...
                None => {
                    if let Some(best_solution) = best_solution {
                        info!(
                            "Best valid solution was found! Learned {} groups",
            ...
            let action: StackAction<P> = if frame.idx < frame.valid_groups.len() {
                ...
                if current_sequence.len() == self.groups.len() {
                    // A complete solution was found. Record it and continue exploring the
                    // remaining dependency-pruned branches instead of returning immediately.
                    let ordering = utils::finalize_ordering(&self.groups, &current_sequence);
                    if best_solution
                        .as_ref()
                        .map(|(_, best_cost)| current_cost < *best_cost)
                        .unwrap_or(true)
                    {
                        info!(
                            "New best valid solution was found! cost={}, learned {} groups",
                            current_cost,
                            self.groups.iter().filter(|g| g.len() > 1).count()
                        );
                        best_solution = Some((ordering, current_cost));
                    }
                    current_sequence.pop();
                    StackAction::Continue
                } else {
            ...
            } else if frame.invalid_groups.is_empty() {
                // This frame has been fully explored and has no invalid group that could teach us
                // a new dependency. Backtrack to the next remaining branch.
                StackAction::Pop
            } else {
                ...
                match self.find_dependency(
                    ...
                    None => {
                        // No dependency group could be found. Keep backtracking; exhaustive TRTA
                        // may still have a complete solution in another dependency-pruned branch.
...
impl<P> OptimizerTRTAExhaustive<P>
where
    P: SoftPolicy + Clone,
{
